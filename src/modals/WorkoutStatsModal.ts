...
import { WorkoutStatisticsService, WorkoutStatistics } from '../utils/workoutStatisticsService';
...
export class WorkoutStatsModal extends Modal {
	...
	constructor(app: App, plugin: WorkoutTrackerPlugin) {
		super(app);
		this.plugin = plugin;
		this.fileService = new WorkoutFileService(app, plugin.settings.defaultWorkoutFolder);
	...
	private renderStatistics(container: HTMLElement, stats: WorkoutStatistics) {
		...
		new Setting(overviewSection)
			...
			.setDesc(`${stats.workoutStreak} day${stats.workoutStreak !== 1 ? 's' : ''}`);
		...
		if (Object.keys(stats.personalRecords).length > 0) {
			...
			Object.entries(stats.personalRecords).forEach(([exercise, record]) => {
				new Setting(prSection)
					.setName(exercise)
					.setDesc(`${record.weight} lbs × ${record.reps} reps (${record.date})`);
			});
		}
		...
		const recentDates = Object.keys(stats.workoutsByDate)
			.sort()
			.slice(-7); // Last 7 days with workouts
		...
		if (recentDates.length > 0) {
			recentDates.forEach(date => {
				...
				new Setting(recentSection)
					.setName(date)
					.setDesc(`${workouts.length} workout${workouts.length !== 1 ? 's' : ''}: ${workouts.map(w => w.name).join(', ')}`);
			});
		} else {
		...
		new Setting(container)
			.addButton(btn => btn
				.setButtonText('Refresh Statistics')
				.onClick(async () => {
					container.empty();
					container.createEl("h2", { text: "Workout Statistics" });
					const loadingEl = container.createDiv({ text: "Loading statistics..." });
